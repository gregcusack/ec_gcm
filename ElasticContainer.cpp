...
uint64_t ec::ElasticContainer::decr_rt_remaining(uint64_t _slice) {
    runtime_remaining -= _slice;
    return runtime_remaining;
}

uint64_t ec::ElasticContainer::incr_rt_remaining(uint64_t give_back) {
...
int ec::ElasticContainer::handle_bandwidth(const msg_t *req, msg_t *res) {
...
int ec::ElasticContainer::handle_slice_req(const ec::msg_t *req, ec::msg_t *res, int clifd) {
    if(req == nullptr || res == nullptr) {
...
int ec::ElasticContainer::handle_mem_req(const ec::msg_t *req, ec::msg_t *res, int clifd) {
    if(req == nullptr || res == nullptr) {
...
int ec::ElasticContainer::add_cgroup_to_ec(msg_t *res, const uint32_t cgroup_id, const uint32_t ip, int fd) {
    if(!res) {
...
const std::vector<ec::Agent *> &ec::ElasticContainer::get_agents() const {
    return agents;
}

uint64_t ec::ElasticContainer::reclaim_memory(int client_fd) {
    int j = 0;
